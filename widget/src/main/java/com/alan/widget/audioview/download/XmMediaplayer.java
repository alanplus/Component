...
import com.alan.httplib.XmHttpExecutor;
...
//        }
    ...
    public void play(String name, IDownloadConfig iDownloadConfig, IMediaStateChangeListener iMediaStateChangeListener) {
        ...
        XmHttpExecutor.file(iDownloadConfig.getUrl())
                .setTargetFile(iDownloadConfig.getDestName(name))
                .setOnFileHttpCallBack(new BaseRequest.OnFileHttpCallBack() {
                    @Override
                    public void onProgressCallback(long progress, long len) {

                    }

                    @Override
                    public void onSuccess(String content, File file) {
                        setState(IMediaStateChangeListener.STATE_LOADFINISH, 0);
                        play(file.getPath(), AUDIO_FILE_TYPE_FILE);
                    }
